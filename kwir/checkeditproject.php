<?php
	...
	if((!isset($_POST['id'])) || (!isset($_POST['title'])) || (!isset($_POST['descrip'])) || (!isset($_POST['conditions'])) || (!isset($_POST['producer'])) || (!isset($_POST['link']))) {
	...
	$descrip = htmlentities($descrip, ENT_QUOTES, "UTF-8");
	$descrip = mysqli_real_escape_string($con, $descrip);
	
	$conditions = $_POST['conditions'];
	$conditions = htmlentities($conditions, ENT_QUOTES, "UTF-8");
	$conditions = mysqli_real_escape_string($con, $conditions);
	
	$producer = $_POST['producer'];
	$producer = htmlentities($producer, ENT_QUOTES, "UTF-8");
	...
	$sql = "update projects set title = '".$title."' , descrip = '".$descrip."' , conditions = '".$conditions."' , producer = '".$producer."' , link = '".$link."' where id = ".$id;
	if (mysqli_query($con, $sql)) 
	{
		$info = "Udało się zauktualizować projekt.";
	}
	else 
	{
		$info = "Nie udało się zaktualizować projektu. Coś poszło nie tak:(";
	}
